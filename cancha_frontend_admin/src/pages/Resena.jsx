...
const Resena = () => {
  ...
  const fetchResenas = async (params = {}) => {
    ...
    console.log('🔍 Parámetros de búsqueda:', fullParams); // ← DEBUG
    
    try {
      let response;
      if (params.q) {
        console.log('📝 Buscando por texto:', params.q);
        response = await api.get('/resena/buscar', { params: fullParams });
      } else if (params.tipo) {
        console.log('🎯 Aplicando filtro:', params.tipo);
      ...
      console.log('✅ Respuesta recibida:', response.data); // ← DEBUG
