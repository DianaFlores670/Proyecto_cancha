...
const permissionsConfig = {
    ...
    ADMIN_ESP_DEP: { canView: true, canApprove: false, canReject: false },
...
const getEffectiveRole = () => {
    try {
        ...
        if (bag.size === 0 && token && token.split(".").length === 3) {
            try {
                ...
                if (Array.isArray(payload.roles)) {
                    for (const r of payload.roles) bag.add(r);
                }
                if (payload.rol) bag.add(payload.rol);
                if (payload.role) bag.add(payload.role);
            } catch { }
        }
...
const SolicitudEncargado = () => {
    const basePath = "/solicitud-encargado";
                    ...
                    prev.map(row =>
                        row.id_solicitud === current.id_solicitud
                            ? {
                                ...row,
                                estado: "aprobada",
                                comentario_decision: approveComment || row.comentario_decision
                            }
                            : row
                    )
                );

                // cerrar el modal DESPUÉS de actualizar y poner el mensaje
                setTimeout(() => {
                    closeApprove();
                }, 300);

                // recargar desde BD para dejar limpio
                fetchSolicitudes(estado ? { estado } : {});
            } else {
                    ...
                    prev.map(row =>
                        row.id_solicitud === current.id_solicitud
                            ? {
                                ...row,
                                estado: "rechazada",
                                comentario_decision: rejectComment || row.comentario_decision
                            }
                            : row
                    )
                );

                // cerrar modal con delay para que el usuario vea "ok!"
                setTimeout(() => {
                    closeReject();
                }, 300);
    ...
    if (!permissions.canView) return <p>No tienes permisos para ver solicitudes.</p>;
                                    ...
                                    <tr key={row.id_solicitud} className="border-t">
                                        ...
                                        <td className="px-4 py-2 flex gap-2">
                                            ...
                                            <button
                                                ...
                                                disabled={!canAct(row)}
                                                className={`text-blue-600 hover:text-blue-800 ${!canAct(row) ? "opacity-40 cursor-not-allowed" : ""
                                                    }`}
                                            >
                                                Aprobar
                                            </button>
                                            ...
                                            <button
                                                ...
                                                disabled={!canAct(row)}
                                                className={`text-red-600 hover:text-red-800 ${!canAct(row) ? "opacity-40 cursor-not-allowed" : ""
                                                    }`}
                                            >
                                                Rechazar
                                            </button>
